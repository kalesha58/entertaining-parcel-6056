import React from "react";
import { useEffect } from "react";
import axios from "axios";
import "./Search.css";
import { useState } from "react";
import {Link} from "react-router-dom"

const Search = () => {
  const { search } = JSON.parse(localStorage.getItem("search"));
  const [d,setD]=useState([])
  useEffect(() => {
    axios
      .get(
        `https://newsapi.org/v2/everything?q=Apple&from=2022-09-29&sortBy=popularity&apiKey=0e98c87ef3af46d3be3d8d1615744850`
      )
      .then((r) =>  setD(r.data.articles))
  },[]);
  return <div className="main_content">
    <div>

    {d.map((e)=>((e.source.name[0].toLowerCase()==search[0])&&(
        <div key={e.id} className="container">
            
            {console.log(e)}
            <div className="search_link" >

                <Link >{e.url}</Link>
            </div>
            <div className="l_box">

                <img className="search_img" src={e.urlToImage} alt="url" />
                  <p className="search_content">{e.description}</p>
            </div>
        </div>)
    ))}
    </div>
    <div className="main_content_2">

    </div>
  </div>;
};

export default Search;
